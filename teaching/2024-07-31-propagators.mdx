...
>
  <b>Transversal and longitudinal cuts to a Gaussian beam</b>
</Figure>
...
Note that in computing equation (4), one pre-computes $H(f_{x_1},f_{y_1})$ and spares a calculation of the Fourier Transform. 
...
A less well-known equivalent geometry [6] puts instead the reference on the pixel size $D$ of the output plane. In that case, an upscaling happens:
